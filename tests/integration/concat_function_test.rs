...
fn test_concat_single_string() {
    let result = Eval::eval_function("CONCAT", vec![Value::Text("single".to_string())]).unwrap();
...
fn test_concat_single_int() {
    let result = Eval::eval_function("CONCAT", vec![Value::Int(999)]).unwrap();
...
fn test_concat_complex_pet_store() {
    ...
    for (inputs, expected) in test_cases {
        let args: Vec<Value> = inputs.into_iter().map(|s| Value::Text(s.to_string())).collect();
...
fn test_substring_start_zero_treated_as_one() {
    let result = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("hello".to_string()), Value::Int(0), Value::Int(2)],
    )
    .unwrap();
...
fn test_substring_length_out_of_bounds() {
    let result = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("hello".to_string()), Value::Int(1), Value::Int(100)],
    )
    .unwrap();
...
fn test_substring_start_out_of_bounds() {
    let result = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("hello".to_string()), Value::Int(10), Value::Int(2)],
    )
    .unwrap();
...
fn test_substring_empty_string() {
    let result = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("".to_string()), Value::Int(1), Value::Int(5)],
    )
    .unwrap();
...
fn test_substring_email_masking() {
    ...
    let result =
        Eval::eval_function("SUBSTRING", vec![email, Value::Int(1), Value::Int(5)]).unwrap();
...
fn test_upper_mixed_case() {
    let result =
        Eval::eval_function("UPPER", vec![Value::Text("Hello World".to_string())]).unwrap();
...
fn test_upper_empty_string() {
    let result = Eval::eval_function("UPPER", vec![Value::Text("".to_string())]).unwrap();
...
fn test_upper_with_numbers() {
    let result = Eval::eval_function("UPPER", vec![Value::Text("abc123def".to_string())]).unwrap();
...
fn test_lower_mixed_case() {
    let result =
        Eval::eval_function("LOWER", vec![Value::Text("Hello World".to_string())]).unwrap();
...
fn test_lower_empty_string() {
    let result = Eval::eval_function("LOWER", vec![Value::Text("".to_string())]).unwrap();
...
fn test_length_empty_string() {
    let result = Eval::eval_function("LENGTH", vec![Value::Text("".to_string())]).unwrap();
...
fn test_length_with_spaces() {
    let result =
        Eval::eval_function("LENGTH", vec![Value::Text("hello world".to_string())]).unwrap();
...
fn test_length_sku_validation() {
    ...
    let result = Eval::eval_function("LENGTH", vec![Value::Text("DF001".to_string())]).unwrap();
...
fn test_coalesce_first_value() {
    let result = Eval::eval_function(
        "COALESCE",
        vec![Value::Text("first".to_string()), Value::Null, Value::Text("second".to_string())],
    )
    .unwrap();
...
fn test_coalesce_single_value() {
    let result = Eval::eval_function("COALESCE", vec![Value::Text("only".to_string())]).unwrap();
...
fn test_coalesce_single_null() {
    let result = Eval::eval_function("COALESCE", vec![Value::Null]).unwrap();
...
fn test_nullif_both_null() {
    let result = Eval::eval_function("NULLIF", vec![Value::Null, Value::Null]).unwrap();
...
fn test_nullif_integers_equal() {
    let result = Eval::eval_function("NULLIF", vec![Value::Int(42), Value::Int(42)]).unwrap();
...
fn test_nullif_integers_different() {
    let result = Eval::eval_function("NULLIF", vec![Value::Int(42), Value::Int(43)]).unwrap();
...
fn test_concat_then_upper() {
    ...
    let concat_result = Eval::eval_function(
        "CONCAT",
        vec![
            Value::Text("hello".to_string()),
            Value::Text(" ".to_string()),
...
fn test_substring_then_lower() {
    ...
    let substring_result = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("HELLO WORLD".to_string()), Value::Int(1), Value::Int(5)],
    )
    .unwrap();
...
fn test_pet_store_full_workflow() {
    ...
    let display_name = Eval::eval_function(
        "CONCAT",
        vec![
    ...
    let upper_category =
        Eval::eval_function("UPPER", vec![Value::Text("food".to_string())]).unwrap();
    assert_eq!(upper_category, Value::Text("FOOD".to_string()));

    // 3. Get masked email for display
    let masked_email = Eval::eval_function(
        "SUBSTRING",
        vec![Value::Text("customer@example.com".to_string()), Value::Int(1), Value::Int(8)],
    )
    .unwrap();
    assert_eq!(masked_email, Value::Text("customer".to_string()));

    // 4. Validate SKU length
    let sku_length = Eval::eval_function("LENGTH", vec![Value::Text("DF001".to_string())]).unwrap();
    assert_eq!(sku_length, Value::Int(5));

    // 5. Handle null price with coalesce
    let price_display = Eval::eval_function(
        "COALESCE",
        vec![Value::Null, Value::Text("Call for price".to_string())],
    )
    .unwrap();
