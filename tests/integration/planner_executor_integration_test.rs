...
use vaultgres::parser::ast::{Expr, SelectStmt, JoinClause, JoinType, BinaryOperator, ColumnDef, DataType};
use std::sync::Arc;
...
fn assert_column_name_valid(column_name: &str, expr_debug: &str) {
    assert!(
        ...
        "Column name '{}' contains '{{' from expression: {}", column_name, expr_debug
    );
    assert!(
        !column_name.contains('}'), 
        "Column name '{}' contains '}}' from expression: {}", column_name, expr_debug
    ...
    assert!(
        ...
        "Column name '{}' contains type name from expression: {}", column_name, expr_debug
    );
    assert!(
        !column_name.contains("Expr::"), 
        "Column name '{}' contains enum prefix from expression: {}", column_name, expr_debug
    );
    assert!(
        !column_name.is_empty(),
        "Column name is empty for expression: {}", expr_debug
    );
}
    ...
    let select_stmt = SelectStmt {
        ...
        joins: vec![
            JoinClause {
                ...
                on: Expr::BinaryOp {
                    left: Box::new(Expr::QualifiedColumn { table: "c".to_string(), column: "id".to_string() }),
                    op: BinaryOperator::Equals,
                    right: Box::new(Expr::QualifiedColumn { table: "o".to_string(), column: "customer_id".to_string() }),
                },
            },
        ],
        where_clause: None,
...
fn test_column_name_invariants_for_all_expr_types() {
    ...
    let test_cases = vec![
        ...
        ("QualifiedColumn", Expr::QualifiedColumn { 
            table: "t".to_string(), 
...
fn test_view_with_qualified_columns_planning() {
    ...
    let view_stmt = SelectStmt {
        ...
        columns: vec![
            ...
            Expr::Alias {
                ...
                expr: Box::new(Expr::QualifiedColumn { table: "o".to_string(), column: "id".to_string() }),
            },
            Expr::QualifiedColumn { table: "o".to_string(), column: "total".to_string() },
        ],
        from: "customers".to_string(),
    ...
    assert!(plan_result.is_ok(), 
        "SELECT from view with qualified columns failed: {:?}", plan_result.err());
}
...
fn test_malformed_column_name_detection() {
    ...
    for name in malformed_names {
        ...
        assert!(
            ...
            "Test setup error: '{}' should be detected as malformed", name
        );
    }
    ...
    for name in valid_names {
        assert!(
            ...
            !name.contains("QualifiedColumn") && !name.contains("Expr::"),
            "Test setup error: '{}' incorrectly flagged as malformed", name
        );
    }
}
