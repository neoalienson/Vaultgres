...
fn test_compare_simple_select() {
    ...
    println!(
        "VaultGres: {:?}, Postgres: {:?}, Speedup: {:.2}x",
        vaultgres_time, postgres_time, speedup
    );
...
fn test_compare_join_performance() {
    ...
    for db in [env.vaultgres(), env.postgres()] {
        ...
        for i in 0..1000 {
            ...
            db.execute(&format!("INSERT INTO orders VALUES ({}, {}, {})", i, i % 100, i * 10))
                .unwrap();
        }
    }
    ...
    let speedup = postgres_time.as_secs_f64() / vaultgres_time.as_secs_f64();
    println!(
        "Join - VaultGres: {:?}, Postgres: {:?}, Speedup: {:.2}x",
        vaultgres_time, postgres_time, speedup
    );
}
...
fn test_compare_aggregation() {
    ...
    let speedup = postgres_time.as_secs_f64() / vaultgres_time.as_secs_f64();
    println!(
        "Aggregation - VaultGres: {:?}, Postgres: {:?}, Speedup: {:.2}x",
        vaultgres_time, postgres_time, speedup
    );
}
