...
fn test_memory_leak_24h() {
    ...
    for i in 0..86400 {
        db.execute(&format!("INSERT INTO events VALUES ({}, 'data{}', NOW())", i, i)).ok();
    ...
    assert!(
        metrics.memory_growth_mb < 100.0,
...
fn test_disk_growth_48h() {
    let env = TestEnv::new().with_vaultgres().with_persistence().start();
    ...
    for hour in 0..48 {
        for i in 0..3600 {
            db.execute(&format!(
                "INSERT INTO logs VALUES ({}, 'log message {}')",
                hour * 3600 + i,
                i
            ))
            .ok();
